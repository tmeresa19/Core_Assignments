...
# we can also say "from flask_app.__init__ import DATABASE" but the init file will automatically be detected so no need to mention it there
...
class User:
    ...
    def get_one_by_email(cls, data):
        ...
        if len(result)>0:
            if option == "registration":
                flash("Email already in use, please select another one.", "email_error")
                return False
            if option == "login":
                return cls(result[0])
        else:
    ...
    def validate_registration(data):
        ...
        if len(data['first_name']) < 2:
            flash('First name must be at least 2 characters long.', 'first_name_error')
            is_valid = False
    ...
    @staticmethod
    def validate_password(pwd, encrypted_pwd):
